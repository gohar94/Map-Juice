    ...
    public ArrayList<String> findNodesToDelete() {
        ...
        try {
            for (String str : master_list) {
                ...
                logger.finest("Sending getWhereD");
                output.writeUTF("getWhereD " + sdfsFileName);
                String reply = input.readUTF();
                logger.finest("Reply for getWhereD " + sdfsFileName + " is " + reply);
                if (reply.equals("abort")) {
                    logger.finest("File is not present in SDFS - master didn't tell where to delete from"); 
