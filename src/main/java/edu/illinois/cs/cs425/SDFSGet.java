...
public class SDFSGet extends Thread {
    ...
    public void run() {
        try {
            ...
            if (nodes.size() == 0) {
                logger.finest(sdfsFileName + " does not exist in SDFS"); 
                return;
            } else {
                boolean sucess = readFromNodes(nodes);
                if(sucess==false){
                    logger.info("Get failed, do get again");
                    Thread.sleep(1000);
                    nodes = findNodesToRead();
                    readFromNodes(nodes);
    ...
    public ArrayList<String> findNodesToRead() {
        ...
        try {
            for (String str : master_list) {
                ...
                logger.finest("JBTW master is " + str);
                socket = new Socket(ip, portNumber);
                input = new DataInputStream(socket.getInputStream());
                output = new DataOutputStream(socket.getOutputStream());
                logger.finest("Sending getWhere");
                output.writeUTF("getWhere " + sdfsFileName);
                String reply = input.readUTF();
                logger.finest("Reply for getWhere " + sdfsFileName + " is " + reply);
                if (reply.equals("abort")) {
                    logger.finest("File is not present in SDFS - master didn't tell where to read from"); 
                    return nodesToRead;
                }
    ...
    public boolean readFromNodes(ArrayList<String> nodes) {
        ...
        for (String node : nodes) {
            ...
            try {
                ...
                if (file.exists()) {
                    if (file.isFile()) {
                        logger.finest("File fetched into local file system");
                        success = true;
                    }
                }
            } catch (Exception e) {
                logger.severe(e.toString());
                logger.severe("nodes die");
            } finally {
                try {
                    if (output != null)
                        output.close();
