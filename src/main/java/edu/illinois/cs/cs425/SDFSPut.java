...
// public class SDFSPut implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (nodes.size() == 0) {
                logger.finest("Not writing to any nodes - finishing this thread!");
                return;
            }
            int successfulReplicas = writeToNodes(nodes);
            logger.finest(sdfsFileName + " written to " + successfulReplicas + " nodes");
            if (successfulReplicas == Math.min(nodes.size(), 3)) {
                logger.finest("All replicas have written the file " + sdfsFileName);
            }
        } catch(Exception e) {
    ...
    public ArrayList<String> findNodesToWrite() {
        ...
        ArrayList<String> tempMasters = new ArrayList<String>(master_list);
        long seed = System.nanoTime();
        Collections.shuffle(tempMasters, new Random(seed));
        for (String str : tempMasters) {
            Socket socket = null;
            DataInputStream input = null;
            DataOutputStream output = null;
            try {
                logger.finest("Trying master " + str);
                int basePort = Integer.parseInt(str.split("//")[3]);
                Random rand = new Random();
                int randomNum = rand.nextInt((9 - 0) + 1) + 0;
                socket = new Socket(str.split("//")[0],basePort+randomNum);
                input = new DataInputStream(socket.getInputStream());
                output = new DataOutputStream(socket.getOutputStream());
                logger.finest("Sending putWhere");
                output.writeUTF("putWhere " + sdfsFileName);
                String reply = input.readUTF();
                logger.finest("Reply for putWhere " + sdfsFileName + " is " + reply);
                if (reply.equals("abort")) {
                    logger.finest("File is already in SDFS - master did not tell where to write!");
                    return nodesToWrite;
                }
    ...
    public int writeToNodes(ArrayList<String> nodes) {
        ...
        for (String node : nodes) {
            logger.finest("Writing on node " + node);
            SDFSWriter sw = new SDFSWriter(master_list, node, localFileName, sdfsFileName, doneSignal);
            sw.start();
        }

        try {
            doneSignal.await(10, TimeUnit.SECONDS); // TODO timeout check
        } catch (Exception e) {
            logger.severe(e.toString());
        }
