...
public class MessageHandler extends Thread{
	...
	public MessageHandler(DatagramSocket _server, DatagramPacket _receivePacket, CopyOnWriteArrayList<String> _members, CopyOnWriteArrayList<String> _master_list,CopyOnWriteArrayList<String> _files,boolean _leader, String _timestamp, int _portnumber,int _sdfsPort, int _mjPort) { 
      ...
      public void printMemberlist()
      {
        try{
            ...
            System.out.println("*******************************");
            int i=0;
            String selfID = InetAddress.getLocalHost().getHostName()+"//"+Integer.toString(portnumber)+"//"+timestamp+"//"+Integer.toString(sdfsPort)+"//"+Integer.toString(mjPort);
		...
		try {
			logger.finest("MessageHandler started");
                  ...
                  if (jsonArgument.has("join")) { 
                        ...
                        if (leader) {
                            logger.info("Adding worker to list " + target);
                            Daemon.workers.add(target); // TODO is this the right place to add the worker to worker list when it first joins?
                            logger.info("Worker list size right now is " + Daemon.workers.size());
                              // Put all members in list to a JSONObject. Including leader itself
                              if(jsonArgument.has("master")){
                                master_flag = true;
                                logger.finest("This is a master");
                              }
                              if(master_flag==true){
                              ...
                              String leaderID = InetAddress.getLocalHost().getHostName()+"//"+Integer.toString(portnumber)+"//"+timestamp+"//"+Integer.toString(sdfsPort)+"//"+Integer.toString(mjPort);
                              ...
                              if(master_flag==true){
                                ...
                                logger.finest("About to put files");
                                for(String file: files){
                                  logger.finest("Putting file " + file);
                                  fileLst_array.put(file);
                                }
                                jsonAck.put("init_f",fileLst_array);
                                logger.finest("File list prepared");
                              }
                    ...
                    if(jsonArgument.has("init_m")){
                      ...
                      if(jsonArgument.has("init_f")){
                        ...
                        logger.finest("receiving inital file list from master");
                        for(int i=0;i<file_list.length();i++){
                          String fileName = file_list.getString(i);
                          files.add(fileName);
                        }
                        logger.finest("finish setting up file list");
                      }
                    } 
                        ...
                        if (jsonArgument.has("leave")) {
                              ...
                              if(members.contains(target))
                              {
                                    ...
                                    logger.finest("Remove member "+target+" status "+status);
                                    printMemberlist();
                                    /*logger.finest("Request file list clear");
