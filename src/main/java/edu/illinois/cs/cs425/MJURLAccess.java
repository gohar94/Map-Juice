...
public class MJURLAccess {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<MJURLAccessThread> threads = new ArrayList<MJURLAccessThread>();
        logger.info("Spawning 10 thread to count word");
        for (int i = 0; i < 10; i++) {
            String inputFile = inputFilePrefix +"_"+Integer.toString(i) + ".in";
            //TODO make directory
            MJURLAccessThread mjt = new MJURLAccessThread(inputFile, outputFilePrefix+"_"+timeStamp, i);
            mjt.start();
            threads.add(mjt);
        }

        try {
            for (MJURLAccessThread thread : threads) {
                thread.join();
            }
...
class MJURLAccessThread extends Thread {
    String inputFile;
    String outputFilePrefix;
    int id;
    private Object lock = new Object();
    
    public MJURLAccessThread(String _inputFile, String _outputFilesPrefix, int _id) {
    ...
    public static synchronized void write(HashMap<String, Integer> map, String outputFilePrefix, int id) {
        try {
            ...
            while (it.hasNext()) {
                ...
                //fileNameOutput = fileNameOutput.replaceAll("\\W+", "");
