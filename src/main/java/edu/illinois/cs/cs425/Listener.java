...
public class Listener extends Thread {
	...
	public Listener(int _portNumber,CopyOnWriteArrayList<String> _members,CopyOnWriteArrayList<String> _master_list,CopyOnWriteArrayList<String> _files,boolean _leader, String _timestamp,int _sdfsPort, int _mjPort) {
		members = _members;
	...
	public void run(){	
		try {
            ...
            server.setSoTimeout(10000); // Just to go back and see if this thread is even alive or not, otherwise go back to waiting
            while (!Thread.currentThread().isInterrupted() && !finished) {
				...
				if (receivePacket != null) {
					...
					MessageHandler messagehandler = new MessageHandler(server,receivePacket,members,master_list,files,leader,timestamp,portNumber,sdfsPort,mjPort);
