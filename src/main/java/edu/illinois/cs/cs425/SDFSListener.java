...
public class SDFSListener extends Thread {
    ...
    public SDFSListener(int portNumber, CopyOnWriteArrayList<String> _members, CopyOnWriteArrayList<String> _master_list, CopyOnWriteArrayList<String> _files) { 
        try {
            ...
            logger.finest("SDFS Lisener could not be started");
    ...
    public void run() {
        try {
            ...
            logger.finest("SDFS Listener started listening on " + listener.getInetAddress() + ":" + listener.getLocalPort());

            while (!Thread.currentThread().isInterrupted() && !finished) {
                Socket socket = listener.accept();
                logger.finest("Connected to " + socket.getRemoteSocketAddress());
                
                logger.finest("Spawning thread to handle incoming request to SDFS Listener");
                SDFSMessageHandler handler = new SDFSMessageHandler(socket, members, master_list, files);
                handler.start();
            }
        } catch(Exception e) {
            logger.finest("Problem in SDFS Listener thread - killing this thread");
            logger.severe(e.toString());
        }
    }
}
