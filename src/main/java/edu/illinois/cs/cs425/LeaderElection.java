...
public class LeaderElection extends Thread{
	...
	logger.finest("New master has been elected!"+'\n'+" Start broadcasting the good news!");
		for(String member : members){
			...
			try{
				...
				dos.flush();
				logger.finest("New master broadcast send to: "+member);
			}catch(Exception e){
	...
	public void run(){
		try{	
			...
			logger.finest("Find master failure: "+Integer.toString(failureNumber));
			...
			for (String member : members) {
				if(!master_list.contains(member)){
					...
					logger.finest(member+" is already in master list");
				}
				
			}
			for(int i=0;i<failureNumber;i++){
				// TODO send out file scp?
				logger.finest("Select new master candidate: " +candidate[i]);
				...
				dos.flush();
				logger.finest("Inform candidate!");
				// TODO change this
				
	            
	            logger.finest("Starting sending log to candidate: "+master_Ip+"//"+Integer.toString(master_Port));
	            String filesToSend = "";
	            ...
	            dos.flush();
				logger.finest("Finish sending log file to candidate: "+candidate[i]);
