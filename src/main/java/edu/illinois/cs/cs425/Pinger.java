...
public class Pinger extends Thread {
     ...
     */
    public Pinger(CopyOnWriteArrayList<String> _members,CopyOnWriteArrayList<String> _master_list, String _timestamp,int _port,int _sdfsPort, int _mjPort) {
        members = _members;
    ...
    public void informMaster(String target){
        try{
            ...
            dos.flush();
            logger.finest("Informed master: "+topMaster+"[Failure of "+target+"]");
            sdfsClient.close();
            dos.close();
        }catch(Exception e){
    ...
    public void printMemberlist()
      {
        try{
            ...
            System.out.println("*******************************");
            int i=0;
            String selfID = InetAddress.getLocalHost().getHostName()+"//"+Integer.toString(port+1)+"//"+timestamp+"//"+Integer.toString(sdfsPort)+"//"+Integer.toString(mjPort);
    ...
    public void run() {
        try {
            ...
            while (!Thread.currentThread().isInterrupted() && !finished) {
                ...
                Thread.sleep(1000); // TODO Fix this value later, changing for debugging
    ...
    public void pingMember(String member, int index) {
        ...
        try {
            ...
            clientSocket.setSoTimeout(10000); // Timeout period to wait for ACK 100ms - otherwise mark member as suspicious
                ...
                if (members.contains(target)) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        try {
                            ...
                            logger.finest("Detect failure! Inform Master!");
