...
public class Daemon {
    ...
    public static ConcurrentLinkedQueue<MJJob> jobs = new ConcurrentLinkedQueue<MJJob>();
    public static ConcurrentLinkedQueue<MJTask> tasks = new ConcurrentLinkedQueue<MJTask>();
    public static CopyOnWriteArrayList<String> workers = new CopyOnWriteArrayList<String>();
    public static ConcurrentHashMap<String,MJTask> allTasks = new ConcurrentHashMap<String,MJTask>();
    ...
    public static void init() {
        try {
            ...
            process1.waitFor();
            logger.finest("Build folder for sdfs file");
    ...
    public static void main(String[] args) {
        try {
            ...
            Option p3 = new Option("p3","port of maplejuice",true,"mjPort");
            ...
            if(commandLine.hasOption("p3")){
                logger.finest("Option p3 is found!");
                mjPort = Integer.parseInt(commandLine.getOptionValue("p3"));
            }
            if(commandLine.hasOption("m")){
                logger.finest("User specified master");
                master = true;
                // leader will add itself as the first master in the master list
                if(leader==true){
                    String master_ID = localhostName.getHostName()+"//"+Integer.toString(listenerPort)+"//"+timeStamp+"//"+Integer.toString(sdfsPort)+"//"+Integer.toString(mjPort);
            ...
            if (commandLine.hasOption("r")) {
                ...
                String newleader = localhostName.getHostName()+"//"+Integer.toString(listenerPort)+"//"+timeStamp+"//"+Integer.toString(sdfsPort)+"//"+Integer.toString(mjPort);
                broadCast("join",newleader);
                logger.finest("Broad cast new leader!");
            } 
            
            selfID = InetAddress.getLocalHost().getHostName()+"//"+Integer.toString(listenerPort)+"//"+timeStamp+"//"+Integer.toString(sdfsPort)+"//"+Integer.toString(mjPort);

            logger.finest("Daemon start a listener: "+localhostName.getHostName()+":"+Integer.toString(listenerPort));
            Listener listener = new Listener(listenerPort,members,master_list,files,leader,timeStamp,sdfsPort,mjPort);
            listener.start();
        
            logger.finest("Daemon start a pinger: "+localhostName.getHostName()+":"+Integer.toString(pingerPort));
            Pinger pinger = new Pinger(members, master_list,timeStamp,pingerPort,sdfsPort,mjPort);
            pinger.start();

            for (int i = 0; i < 10; i++) {
                logger.info("Starting SDFS Listener " + i + " : "+localhostName.getHostName()+":"+Integer.toString(sdfsPort+i));
                SDFSListener sl = new SDFSListener(sdfsPort+i, members, master_list, files);
                sl.start();
            }

            logger.info("Starting MJ Listener: "+ localhostName.getHostName()+":"+ Integer.toString(mjPort));
            MJListener mjl = new MJListener(mjPort);
            mjl.start();
	    logger.info("Going to leader condition");

            if(leader==true){
		logger.info("Leader condition");
                logger.info("Starting MapJuice Assigner");
                MJAssigner assigner = new MJAssigner();
                assigner.start();
                MJTaskChecker checker = new MJTaskChecker();
                checker.start();
            }
		...
		logger.info("Leader is false");
                ...
                String message = localhostName.getHostName()+"//"+Integer.toString(listenerPort)+"//"+timeStamp+"//"+Integer.toString(sdfsPort)+"//"+Integer.toString(mjPort);
            ...
            while (true) {
                ...
                if (message.equals("l") || message.equals("leave")) {
                    ...
                    String myID = localhostName.getHostName()+"//"+Integer.toString(listenerPort)+"//"+timeStamp+"//"+Integer.toString(sdfsPort)+"//"+Integer.toString(mjPort);
                    ...
                    System.out.println("*******************************");
                    if(master == true){
                        System.out.println("M:"+localhostName.getHostName()+"//"+Integer.toString(listenerPort)+"//"+timeStamp+"//"+Integer.toString(sdfsPort)+"//"+Integer.toString(mjPort));
                    }else{
                        System.out.println("  "+localhostName.getHostName()+"//"+Integer.toString(listenerPort)+"//"+timeStamp+"//"+Integer.toString(sdfsPort)+"//"+Integer.toString(mjPort));
                    }
                    System.out.println("*******************************");
                    ...
                    logger.finest("Current master list");
