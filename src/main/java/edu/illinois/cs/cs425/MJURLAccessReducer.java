...
public class MJURLAccessReducer {
    private final static Logger logger = Logger.getLogger(Logger.GLOBAL_LOGGER_NAME);
    public static void main(String[] args) {
    	logger.info("MJURLAccessReducer task start");
        ...
        //syncronize write
        ArrayList<MJURLAccessReducerThread> threads = new ArrayList<MJURLAccessReducerThread>();
        logger.info("Spawning 10 thread to count word");
        for (int i = 0; i < 10; i++) {
            String inputFile = inputFilePrefix +"_"+seqNum+"_"+Integer.toString(i) + "_keyList.txt";
            MJURLAccessReducerThread mjrt = new MJURLAccessReducerThread(inputFileFolder,inputFile,outputFilePrefix+"_"+timeStamp,seqNum,i);
            mjrt.start();
            threads.add(mjrt);
        }
        try {
            for (MJURLAccessReducerThread thread : threads) {
                thread.join();
            }
        }catch(Exception e){
 ...
 class MJURLAccessReducerThread extends Thread {
    	String inputFile;
        ...
        public MJURLAccessReducerThread(String _inputFileFolder, String _inputFile, String _outputFilePrefix, String _seqNum,int _id) {
